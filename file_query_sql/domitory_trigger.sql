...
delimiter $$
drop trigger if exists domitory_management.before_insert_students;
...
delimiter $$
drop trigger if exists domitory_management.before_insert_motorbikes;
...
delimiter $$
drop trigger if exists domitory_management.before_insert_parking_histories;
...
begin
	...
	select count(*) into total from parking_histories pk 
		where pk.student_id=new.student_id and pk.status = 'FREE' 
