...
insert into vistors(student_id, cccd, name, phone, gender, time_in, time_out) value ( (select id from students where msv like 'B18DCVT257'), '001082946357', 'Tran Van Nam', '0983867797', 'MALE', now(), date_add(now(),interval 2 hour));
...
insert into student_service(student_id, service_id, time_use, time_end) value ( (select id from students where msv like 'B18DCVT257'), (select id from services where name like 'Trông xe'), 1, date_add(now(), INTERVAL 1 month ) );
...
insert into motorbikes(student_id, name, license_plate, time_registration, status) value ( (select id from students where msv like 'B18DCVT257'), 'Honda Vision 2021', '29X1 77821', now(), 'REGISTER');
insert into motorbikes(student_id, name, license_plate, time_registration, status) value ( (select id from students where msv like 'B18DCVT257'), 'Honda Wave 2021', '29X1 14245', now(), 'REGISTER');

-- lich su gui xe
insert into parking_histories(student_id, motorbike_id, time_in, time_out, status, price) value ( (select id from students where msv like 'B18DCVT257'), (select id from motorbikes where license_plate like '29X1 77821'), now(), date_add(now(),interval 2 hour), 'FREE',0);